...
import { StorageListService } from '../storage-list/storage-list.service';
import { storageListServiceSpy } from '../storage-list/storage-list.service.spec';
...
describe('StorageTreeControlService', () => {
  ...
  beforeEach(() => {
    selection = jasmine.createSpyObj('_selection', [
      'clear',
      'select',
      'toggle',
      'isSelected',
  ...
  it('should return has _selection (2)', () => {
    selection.hasValue.and.returnValue(true);
  ...
  it('should return selected directory parent root dir', () => {
    ...
    selection.hasValue.and.returnValue(true);
    const nodes: StorageNode[] = [directoryNode, fileNode, rootFile];
    dataSource.data = nodes;

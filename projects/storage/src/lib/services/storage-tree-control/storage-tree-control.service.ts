...
export class StorageTreeControlService extends FlatTreeControl<StorageNode> implements OnDestroy {
  ...
  constructor(
    ...
    super(
      (node: StorageNode) => node.depth - (rootNode.depth + 1),
    ...
    this.subscriptions.push(
      this.eventBus.of(NodeDeletedEvent.CHANNEL).subscribe(this._nodeDeleted.bind(this)),
    );
    this.subscriptions.push(
      this.expansionModel.changed.subscribe(() =>
  ...
  public loadExpansion(data: StorageNode[]) {
    const expanded: StorageNode[] = _.intersectionBy(
      data,
      this.localStorage.getItem(this.expansionId, []),
      'path',
    );
  ...
  public isSelected(node: StorageNode) {
    return (
      !!node &&
      _.findIndex(this._selection.selected, (current) => current.path === node.path) !== -1
    );
  }
  ...
  _toggleSelection($event: MouseEvent, node: StorageNode) {
    ...
    if (
      ...
      (mouseEvent.shiftKey && !this._lastSelection)
    ) {
      this.selectOne(node);
      return;
    }
  ...
  _nodeDeleted(event: NodeDeletedEvent) {
    ...
    _.filter(this._selection.selected, predicate).forEach((node: StorageNode) =>
      this._selection.deselect(node),
    );
  }
}

...
import { KeyBindingsService } from 'projects/commons/src/lib/tools/services/key-bindings/key-bindings.service';
import { keyBindingsServiceSpy } from 'projects/commons/src/lib/tools/services/key-bindings/key-bindings.service.spec';
...
import { StorageTreeControlService } from '../storage-tree-control/storage-tree-control.service';
import { storageTreeControlServiceSpy } from '../storage-tree-control/storage-tree-control.service.spec';
import {
  StorageTreeDataSourceService, STORAGE_ROOT_NODE,
} from '../storage-tree-data-source/storage-tree-data-source.service';
import { storageTreeDataSourceServiceSpy } from '../storage-tree-data-source/storage-tree-data-source.service.spec';
import { StorageTreeScrollService } from '../storage-tree-scroll/storage-tree-scroll.service';
import { storageTreeScrollServiceSpy } from '../storage-tree-scroll/storage-tree-scroll.service.spec';
import { StorageService } from '../storage/storage.service';
import { storageServiceSpy } from '../storage/storage.service.spec';
...
export const storageKeyBindingServiceSpy = () => {
  ...
  const spy = jasmine.createSpyObj('StorageKeyBindingService', ['init', 'upSelection']);
  return spy;
};
...
describe('StorageKeyBindingService', () => {
  ...
  describe('should multi select ', () => {
    ...
    it('down data with lines already selected', () => {
      ...
      expect(treeControl.deselectNode).toHaveBeenCalledWith(
        dataSource._expandedNodes[7],
        dataSource._expandedNodes[8],
      );
    });
  ...
  it('should right selection for directory', () => {
    ...
    dataSource.parentNode
      .withArgs(dataSource._expandedNodes[7])
      .and.returnValue(dataSource._expandedNodes[6]);
  ...
  it('should right selection for file', () => {
    ...
    dataSource.parentNode
      .withArgs(dataSource._expandedNodes[8])
      .and.returnValue(dataSource._expandedNodes[6]);
    service.rightSelection();
  ...
  it('should left selection for directory', () => {
    treeControl._lastSelection = dataSource._expandedNodes[5];
    dataSource.parentNode
      .withArgs(dataSource._expandedNodes[5])
      .and.returnValue(dataSource._expandedNodes[4]);
    treeControl.isExpanded.withArgs(dataSource._expandedNodes[5]).and.returnValue(false);
  ...
  it('should delete selection', () => {
    ...
    expect(storageService.deleteFiles).toHaveBeenCalledWith(
      [dataSource._expandedNodes[7], dataSource._expandedNodes[8]],
      true,
    );
  });

...
import { testStorageDirectoryNode, testStorageFileNode } from '../../entities/storage-node.spec';
...
describe('StorageNodeToPredicatePipe', () => {
  ...
  it('reject other node with close name', () => {
    ...
    expect(
      predicate({ path: 'spotbugs.ext', type: 'FILE', depth: 0, length: 0, lastModified: 0 }),
    ).toBeFalsy();

    expect(
      predicate({ path: 'spotbugs.ext', type: 'FILE', depth: 1, length: 0, lastModified: 0 }),
    ).toBeFalsy();
  });
});

...
import { SecurityConfigurationService } from '../security-configuration/security-configuration.service';
import { securityConfigurationServiceSpy } from '../security-configuration/security-configuration.service.spec';

export const securityServiceSpy = () => {
  // eslint-disable-next-line jasmine/no-unsafe-spy
  const spy = jasmine.createSpyObj('SecurityService', ['init', 'login', 'logout']);
  return spy;
};
...
describe('SecurityService', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      providers: [{ provide: SecurityConfigurationService, useValue: configuration }],
    });

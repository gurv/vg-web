...
export const keyBindingsServiceSpy = () => {
  ...
  const spy = jasmine.createSpyObj('KeyBindingsService', ['add', 'remove']);
  return spy;
};
...
describe('KeyBindingsService', () => {
  ...
  it('should handle event no prevent default & stop prop', () => {
    const binding = new KeyBinding(['ctrl + a'], jasmine.createSpy('binding'), null, false, false, [
      '',
      'TEXTAREA',
      'SELECT',
    ]);
